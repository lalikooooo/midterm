...
const students = [
  ...
  function getSubjectsStatistics(students) {
    ...
    {
        const average = students.map((grades.math + grades.history + grades.physics)/3);
        return average;
    }
  ...
  function getHighAchievers(students, gradeThreshold) {
    const result = students.filter((students)=>
        if(average > gradeThreshold)
    {return students.firstName && students.lastName});}
    
  
  function getMultiAchivers(students, threshold) {
    students.reduce((acc,grades) =>{
        if(students.grades < students.threshold)
        {
         const  object2 = { a:firstName, b=lastName};
         return object2;
